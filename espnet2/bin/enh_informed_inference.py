...
class SeparateSpeech:
    ...
    @torch.no_grad()
    def __call__(
            self, speech_mix: Union[torch.Tensor, np.ndarray], informed: Union[torch.Tensor, np.ndarray], fs: int = 8000, expand_informed: bool=False, informed_ali_ref: torch.Tensor=None
    ) -> List[torch.Tensor]:
        """Inference
        ...
        # lenghts: (B,)
        speech_lengths = speech_mix.new_full(
            [batch_size], dtype=torch.long, fill_value=speech_mix.size(1)
        )
        informed_lengths = informed.new_full(
            [batch_size], dtype=torch.long, fill_value=informed.size(1)
        )

        # a. To device
        speech_mix = to_device(speech_mix, device=self.device)
        speech_lengths = to_device(speech_lengths, device=self.device)
        informed = to_device(informed, device=self.device)
        informed_lengths = to_device(informed_lengths, device=self.device)

        if self.segmenting and speech_lengths[0] > self.segment_size * fs:
            ...
            # b. Enhancement/Separation Forward
            speech_feature_mix, speech_flens = self.enh_model.encoder(speech_mix, speech_lengths)

            # pdb.set_trace()
            if expand_informed:
                informed = make_ali_from_kaldi_ali(informed_ali_ref, self.enh_model.encoder.kernel_size, self.enh_model.encoder.stride, speech_flens)
                informed_lengths = speech_flens

            informed_feature, informed_flens = self.enh_model.informed_encoder(informed, informed_lengths)
            feature_mix, flens, _ = self.enh_model.fusion(speech_feature_mix, informed_feature, speech_flens, informed_flens)
            feature_pre, _, _ = self.enh_model.separator(feature_mix, flens, speech_feature_mix)
            waves = [self.enh_model.decoder(f, speech_lengths)[0] for f in feature_pre]
...
def make_ali_from_kaldi_ali(kaldi_ali, frame_len, frame_shift, real_lens):
...
def inference(
    ...
    loader = EnhancementInformedTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=EnhancementInformedTask.build_preprocess_fn(
            separate_speech.enh_train_args, False
        ),
        collate_fn=EnhancementInformedTask.build_collate_fn(
            separate_speech.enh_train_args, False
        ),
    ...
    for keys, batch in loader:
        ...
        waves = separate_speech(**batch, expand_informed=expand_informed)
