...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            # pdb.set_trace()
            logging.info(f"{iepoch}/{trainer_options.max_epoch}epoch ended")
    ...
    def train_one_epoch(
        ...
        logging.info(f"log interval is {log_interval}")
        model.train()
        ...
        for iiter, (_, batch) in enumerate(
            ...
            # logging.info(f"processing {iiter}")
            ...
            try:
                ...
                if 'out of memory' in str(e):
                    logging.warning(f'ran out of memory, skipping batch {iiter}')
                    ...
                    reporter.next()
                    continue
                elif 'CUDNN_STATUS_EXECUTION_FAILED' in str(e):
                    logging.warning(f'cuDNN failed, skipping batch {iiter}')
                    del batch
                    torch.cuda.empty_cache()
                    gc.collect()
                    reporter.next()
                    ...
                    logging.warning(f"unhandled error on batch {iiter}")
                    raise e
        else:
            if distributed:
