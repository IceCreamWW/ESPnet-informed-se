...
from espnet2.enh.encoder.audio_encoder import AudioEncoder
from espnet2.enh.informed_encoder.text_encoder import EmbeddingEncoder as TextEmbeddingEncoder
...
informed_encoder_choices = ClassChoices(
    name="informed_encoders",
    classes=dict(audio_encoder=AudioEncoder, text_embedding_encoder=TextEmbeddingEncoder),
    type_check=List[AbsInformedEncoder],
    default=["audio_encoder", "text_embedding_encoder"],
)
...
class EnhancementTask(AbsTask):
    ...
    def required_data_names(
        ...
        if not inference:
            retval = ("speech_mix", "speech_ref1", "informed_ref1")
        else:
            # Recognition mode
            retval = ("speech_mix",)
        return retval
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetEnhancementModel:
        ...
        audio_encoder = audio_encoder_choices.get_class(args.audio_encoder)(**args.audio_encoder_conf)
        ...
        model = ESPnetEnhancementInformedModel(
            audio_encoder=audio_encoder, informed_encoder=informed_encoder,
