...
class DPRNNSeparator(AbsSeparator):
    def __init__(
        ...
        self.dprnn = DPRNN(
            ...
            hidden_size=unit,
            output_size=input_dim * num_spk if output_dim is None else output_dim * num_spk,
            dropout=dropout,
            num_layers=layer,
    ...
    def forward(
        self, input: Union[torch.Tensor, ComplexTensor], ilens: torch.Tensor, input_to_mask: torch.Tensor=None
        ...
        processed = processed.view(B, T, N if self.output_dim is None else self.output_dim, self.num_spk)
