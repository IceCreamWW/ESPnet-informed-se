...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        # 3. Submit jobs
        log "Enh collect-stats started... log: '${_logdir}/stats.*.log'"
        ...
        for spk in $(seq "${spk_num}"); do
            ...
            _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/text,informed_spk${spk},sound "
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/spk${spk}.scp,speech_ref${spk},sound "
            _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/text,informed_ref${spk},sound "
        done
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.enh_informed_train \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/${_scp},speech_mix,${_type}" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,text_spk1,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech_mix,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text_spk1,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
        ...
        # Append the num-tokens at the last dimensions. This is used for batch-bins count
        <"${asr_stats_dir}/train/text_spk1_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_spk1_shape.${token_type}"

        <"${asr_stats_dir}/valid/text_spk1_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/text_spk1_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/${_scp},speech_mix,${_type} "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,text_spk1,text "
