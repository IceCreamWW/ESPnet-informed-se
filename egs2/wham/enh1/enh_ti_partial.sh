...
# enh_args="--num_iters_per_epoch 5000"   # Arguments for enhancement model training, e.g., "--max_epoch 10".
...
# scoring_protocol="SI_SNR PESQ STOI SDR SAR SIR"
# scoring_protocol="SI_SNR STOI SDR SAR"
ref_channel=0
...
# token_list=data/token_list/char/tokens.txt
g2p="g2p_en_no_space"
nlsyms_txt=data/nlsyms.txt  # Non-linguistic symbol list if existing.
cleaner=none
...
if ! [ -z "${enh_extra_tag}" ]; then
    enh_tag="${enh_tag}_${enh_extra_tag}"
fi


# The directory used for collect-stats mode
enh_stats_dir="${expdir}/${train_set}_enh_stats_${fs}"
# The directory used for training commands
if [ -z "${enh_exp}" ]; then
enh_exp="${expdir}/${train_set}_enh_${enh_tag}"
fi
...
enh_stats_dir="${expdir}/enh_ali_text_informed_${token_type}_stats_raw_16k_sp_${channels}ch"
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # NOTE: Not applying to test_sets to keep original data
            ...
            cp "${data_feats}/${dset}/text" "${data_feats}/${dset}/text_spk1"
        ...
        log "Generating ${token_type} level token list"
...
#             ${python} -m espnet2.bin.tokenize_text  \
#                 --token_type "char" \
#                 --input "dump/raw/lm_train.txt" --output "data/token_list/char/tokens.txt" \
#                 --field 2- \
#                 --cleaner "none" \
#                 --g2p "none" \
#                 --write_vocabulary true \
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        log "Stage 5: Enh Informed collect stats: train_set=${_enh_train_dir}, valid_set=${_enh_valid_dir}"
        ...
        # 2. Generate run.sh
        log "Generate '${enh_stats_dir}/run.sh'. You can resume the process from stage 9 using this script"
        mkdir -p "${enh_stats_dir}"; echo "${run_args} --stage 9 \"\$@\"; exit \$?" > "${enh_stats_dir}/run.sh"; chmod +x "${enh_stats_dir}/run.sh"

        # 3. Submit jobs
        log "Enh Informed collect-stats started... log: '${_logdir}/stats.*.log'"
        ...
        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/text_phn,informed,text "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/text_phn,informed,text "

        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/ali,informed_ali_ref,text "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/ali,informed_ali_ref,text "

        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/text,informed_text_raw,text_no_process "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/text,informed_text_raw,text_no_process "
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.enh_informed_train \
                --collect_stats true \
        ...
        <"${enh_stats_dir}/train/informed_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${enh_stats_dir}/train/informed_shape.${token_type}"

        <"${enh_stats_dir}/valid/informed_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${enh_stats_dir}/valid/informed_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/text_phn,informed,text "
        _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/informed_shape "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/text_phn,informed,text "
        _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/informed_shape "
        _fold_length_param+="--fold_length ${_fold_length} "

        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/ali,informed_ali_ref,text "
        _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/informed_ali_ref_shape "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/ali,informed_ali_ref,text "
        _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/informed_ali_ref_shape "
        _fold_length_param+="--fold_length ${_fold_length} "
        ...
        ${python} -m espnet2.bin.launch \
            --cmd "${cuda_cmd} --name ${jobname} --mem 30G" \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        # for dset in "${valid_set}"; do
            ...
            # shellcheck disable=SC2086
            fs=16k
            ${python} -m espnet2.bin.enh_informed_inference \
                --ngpu "${_ngpu}" \
                --fs "${fs}" \
                --data_path_and_name_and_type "${_data}/${_scp},speech_mix,${_type}" \
                --data_path_and_name_and_type "${_data}/text,informed,text" \
...
if [ ${stage} -le 100 ] && [ ${stop_stage} -ge 100 ]; then
    _enh_train_dir="${data_feats}/${train_set}"
    _enh_valid_dir="${data_feats}/${valid_set}"
    log "Stage 100: Enh Informed plot attention: train_set=${_enh_train_dir}, valid_set=${_enh_valid_dir}"
    ...
    _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/text,informed,text "
    _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/informed_shape "
    _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/text,informed,text "
    _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/informed_shape "
    _fold_length_param+="--fold_length ${_fold_length} "
    ...
    log "enh plot attentions started... log: '${enh_exp}/plot_att.log'"
    if echo "${cuda_cmd}" | grep -e queue.pl -e queue-freegpu.pl &> /dev/null; then
        # SGE can't include "/" in a job name
        jobname="$(basename ${enh_exp})"
    else
        jobname="${enh_exp}/plot_att.log"
    fi
    # NOTE: --*_shape_file doesn't require length information if --batch_type=unsorted,
    #       but it's used only for deciding the sample ids.
